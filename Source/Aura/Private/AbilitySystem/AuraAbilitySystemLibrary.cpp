...
void UAuraAbilitySystemLibrary::InitizlizeDefaultAttributes(const UObject* WorldContextObject, ECharacterClass CharacterClass, float Level, UAbilitySystemComponent* ASC)
{
	...
	// Make a context handle and apply to it the actor the attributes will be added to
...
void UAuraAbilitySystemLibrary::GiveStartupAbilities(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ECharacterClass CharacterClass)
{
	...
	{
		// Add Abilities set to level 1 - Assumed that common abilities are not level based
	...
	for (TSubclassOf<UGameplayAbility> AbilityClass : DefaultInfo.StartupAbilities)
	{
		...
		{
			// Add Abilities set to the level of the character
...
{
	// World Context Object is the specific game object we want to use: The creature in question
	UCharacterClassInfo* CharacterClassInfo = GetCharacterClassInfo(WorldContextObject);
...
UAbilityInfo* UAuraAbilitySystemLibrary::GetAbililityInfo(const UObject* WorldContextObject)
{
	const AAuraGameModeBase* AuraGameMode = Cast<AAuraGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject));
	if (AuraGameMode == nullptr) return nullptr;
...
// Handles taking a standard GameplayEffectContextHandle and casting it to the custom Aura version that has IsBlockedHit()
...
void UAuraAbilitySystemLibrary::GetLivePlayersWithinRadius(const UObject* WorldContextObject, TArray<AActor*>& OutOverlappingActors, const TArray<AActor*>& ActorsToIngore, float Radius, const FVector& SphereOrigin)
{
	...
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		for (FOverlapResult& Overlap : Overlaps)
		{
			...
			{
				// This line is overkill as Overlap.GetActor() produces the actor, but it just shows that 
				// The CombatInterface has a method of getting an actor too
...
{
	// Return the actors array as there's no reduction to be performed
